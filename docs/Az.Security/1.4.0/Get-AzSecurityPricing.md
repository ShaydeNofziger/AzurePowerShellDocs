...
Gets pricing details of the specific resource ID.
Where ResourceId is one of the IDs returned by \`Get-AzSecurityPricing\`.
...
Gets pricing details of the named Azure Defender plan.
Where \`name\` is one of the names returned by \`Get-AzSecurityPricing\`.
