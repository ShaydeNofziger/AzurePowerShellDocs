...
For Azure PowerShell errors provides  additional detail in debugging service issues, including complete detail about the request and server response  that caused the error.
...
### -Error
One or more error records to resolve. 
If no parameters are specified, all errors in the session are resolved.
