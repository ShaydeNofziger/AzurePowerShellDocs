...
It only supports management plane API and Hostname of Azure Resource Manager is added according to Azure environment setting.
...
Get current signed in user via MicrosoftGraph API.
This example is equivalent to \`Get-AzADUser -SignedIn\`.
...
### -ResourceId
Identifier URI specified by the REST API you are calling.
