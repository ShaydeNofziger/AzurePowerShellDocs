...
\> \[!NOTE\] \> It is discouraged to update configs in multiple PowerShell processes.
Either do it in one process, or make sure the updates are at Process scope (\`-Scope Process\`) to avoid unexpected side-effects.
...
When \`Connect-AzAccount\` the specified subscription will be selected as the default subscription.
...
Sets the "DisplayBreakingChangeWarnings" config as "$false" for "Az.KeyVault" module.
This prevents all the warning messages for upcoming breaking changes in Az.KeyVault module from prompting.
...
Sets the "EnableDataCollection" config as "$true".
This enables sending the telemetry data.
