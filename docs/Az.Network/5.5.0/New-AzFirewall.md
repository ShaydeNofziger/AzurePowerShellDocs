...
This example creates a Firewall attached to virtual hub "vHub".
A firewall policy $fp will be attached to the firewall.
This firewall allows/denies the traffic based on the rules mentioned in the firewall policy $fp.
...
This example creates a Firewall attached to virtual network "vnet" in the same resource group as the firewall.
DNS Proxy is enabled for this firewall and 2 DNS Servers are provided.
Also Require DNS Proxy for Network rules is set so if there are any Network rules with FQDNs then DNS proxy will be used for them too.
...
### -AllowActiveFTP
Allows Active FTP on the Firewall.
...
One or more Public IP Addresses to use for management traffic.
The Public IP addresses must use Standard SKU and must belong to the same resource group as the Firewall.
...
The Public IP addresses must use Standard SKU and must belong to the same resource group as the Firewall.
No input needed for Forced Tunneling Firewalls.
...
### -PublicIpName
Public Ip Name.
The Public IP must use Standard SKU and must belong to the same resource group as the Firewall.
...
### -VirtualNetworkName
Specifies the name of the virtual network for which the Firewall will be deployed.
Virtual network and Firewall must belong to the same resource group.
