...
The      Add-AzVirtualNetworkSubnetConfig is then used to add a subnet to the in-memory representation of the virtual network.
The Set-AzVirtualNetwork command updates the existing virtual      network with the new subnet.
...
Finally, it creates a new subnet with that delegation that is added to the vnet.
The modified configuration is then sent to the server.
