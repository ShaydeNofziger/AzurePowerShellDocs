...
All traffic that matches the conditions identified in $rule1 will be DNAT'ed to translated address and port.
...
Each rule name in a given rule collection must have an unique name and is case insensitive.
...
This example creates a new NAT rule collection with one rule and then gets the rule by name, calling method GetRuleByName on the  rule collection object.
...
This example creates a new NAT rule collection with two rules and then removes the first rule from the rule collection by calling method RemoveRuleByName on the rule collection object.
...
### -Name
Specifies the name of this NAT rule.
