...
### -FqdnTag
Specifies a list of FQDN Tags for this rule.
The available tags can be retrieved using Get-AzFirewallFqdnTag (./Get-AzFirewallFqdnTag.md)cmdlet.
...
The format is \<protocol \`type\>:\<port\>\`. 
For example, "http:80" or "https:443".
Protocol is mandatory when TargetFqdn is used, but it cannot be used with FqdnTag.
...
When used, the asterisk matches any number of characters.
(e.g.
' msn.com' will match msn.com and all its subdomains)
