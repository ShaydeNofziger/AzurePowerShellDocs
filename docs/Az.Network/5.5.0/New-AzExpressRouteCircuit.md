...
This must be a value that is supported by the service provider.
...
### -ServiceProviderName
The name of the circuit service provider.
This must match a name listed by the Get-AzExpressRouteServiceProvider cmdlet.
...
### -SkuFamily
SKU family determines the billing type.
Possible values for this parameter are: \`MeteredData\` or \`UnlimitedData\`.
Note that you can change the billing type from MeteredData to UnlimitedData, but you can't change the type from UnlimitedData to MeteredData.
...
### -SkuTier
The tier of service for the circuit.
Possible values for this parameter are: \`Standard\`, \`Premium\` or \`Local\`.
