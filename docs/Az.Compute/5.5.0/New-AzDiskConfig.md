...
The first command creates a local empty disk object with size 5GB in Standard_LRS storage account type.
...
Create a disk from a Shared Gallery Image Version. 
Id is the id of the shared gallery image version.
Lun is needed only if the source is a data disk.
...
The GalleryImageReference object. 
...
Available values are Standard_LRS, Premium_LRS, StandardSSD_LRS, and UltraSSD_LRS, Premium_ZRS and StandardSSD_ZRS. 
...
### -UploadSizeInBytes
Specifies the size of the contents of the upload including the VHD footer when CreateOption is Upload. 
