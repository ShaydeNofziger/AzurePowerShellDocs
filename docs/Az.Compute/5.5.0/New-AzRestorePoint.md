...
### -ConsistencyMode
ConsistencyMode of the restore point.
Can be specified in the input while creating a restore point.
For now, only CrashConsistent is accepted as a valid input.
...
### -DisksToExclude
List of disk resource Id values that the customer wishes to exclude from the restore point.
