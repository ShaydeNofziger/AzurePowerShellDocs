...
The last command gets an array of recovery points for the item in $backupItem which are ready to be moved to VaultArchive tier and  then stores them in the $rp variable.
...
### -IsReadyForMove
Filters the Recovery Points based on whether RP is Ready to move to target tier.
Use this along with target tier parameter.
...
### -TargetTier
Target tier to check move readiness of recovery point.
Currently only valid value is 'VaultArchive'.
