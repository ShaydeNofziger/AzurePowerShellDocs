...
This command gets the certificate named \`$certName\` from the key vault named \`$vaultName\`.
These commands access secret \`$certName\` and then save the content as a pfx file.
