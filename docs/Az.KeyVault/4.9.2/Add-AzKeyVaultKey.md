...
Use this cmdlet to add keys by using any of the following methods: - Create a key in a hardware security module (HSM) in the Key Vault service.
...
To learn about key versions and the URI structure, see About Keys and Secrets (http://go.microsoft.com/fwlink/?linkid=518560)in the Key Vault REST API documentation.
...
As a best practice, back up your key after it is created or updated, by using the Backup-AzKeyVaultKey cmdlet.
...
This command creates a software-protected EC key named test-key in the key vault named test-kv.
Its curve name is P-256 by default.
...
You can enable it by using the Set-AzKeyVaultKey cmdlet.
...
Generates a key (referred to as a Key Exchange Key (KEK)).
...
Create a secure key in managed hsm named testmhsm.
Its name is test-key and type is RSA.
...
If you import a key by using the KeyFilePath parameter, this parameter is optional: - If you do not specify this parameter, and this cmdlet imports a key that has .byok file name extension, it imports that key as an HSM-protected key.
...
### -Immutable
Sets the release policy as immutable state.
Once marked immutable, this flag cannot be reset and the policy cannot be changed under any circumstances.
...
The acceptable values for this parameter are a comma-separated list of key operations as defined by the JSON Web Key (JWK) specification (http://go.microsoft.com/fwlink/?LinkID=613300): - encrypt
...
When importing BYOK keys, it defaults to 'RSA'.
...
### -ReleasePolicyPath
A path to a file containing JSON policy definition.
