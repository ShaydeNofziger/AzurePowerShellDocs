...
The cmdlet adds the workspace to use the shared integration runtime.
When using \`-SharedIntegrationRuntimeResourceId\` parameter the \`-Type\` must also be included.
Note that the workspace need to be granted permission to use the integration runtime before running cmdlet.
