...
## DESCRIPTION
The Set-AzSynapseTrigger cmdlet creates a trigger in a workspace.
...
Create a new trigger called ContosoTrigger in the workspace ContosoWorkspace.
...
Create a new trigger called ContosoTrigger in the workspace ContosoWorkspace through pipeline.
The trigger is created in the 'Stopped' state, meaning that it doesn't immediately start.
A trigger can be started using the \`Start-AzDataFactoryV2Trigger\` cmdlet.
