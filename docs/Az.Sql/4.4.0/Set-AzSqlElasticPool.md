...
## DESCRIPTION
The Set-AzSqlElasticPool cmdlet sets properties for an elastic pool in Azure SQL Database.
This cmdlet can modify the eDTUs per pool ( Dtu ), storage max size per pool ( StorageMB ), maximum eDTUs per database ( DatabaseDtuMax ), and minimum eDTUs per database ( DatabaseDtuMin ).
...
For details about which values are valid, see the table for your specific size pool in elastic pools (https://learn.microsoft.com/azure/sql-database/sql-database-elastic-pool).
The default values for different editions are as follows: - Basic. 
5 DTUs
...
For more information, see the New-AzSqlElasticPool cmdlet.
...
### -Tags
Specifies a dictionary of Key-value pairs that this cmdlet associates with the elastic pool in the form of a hash table.
For example: \`@{key0="value0";"key 1"=$null;key2="value2"}\`
