...
The Backup storage redundancy used to store backups for the Sql Azure Managed Instance.
...
### -ExternalAdminName
Specifies the display name of the user, group or application which is the Azure Active Directory administrator for the server.
...
Possible values are SystemAsssigned, UserAssigned, 'SystemAssigned,UserAssigned' and None.
...
Managed Instance service price will be discounted for existing SQL Server license owners.
...
Type of Service Principal to be used.
...
### -TimezoneId
The time zone id for the instance to set.
