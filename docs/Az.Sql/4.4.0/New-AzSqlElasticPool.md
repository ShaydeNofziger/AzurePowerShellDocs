...
This command creates an elastic pool in the GengeralPurpose service tier named ElasticPool01.
The server named server01, assigned to an Azure resource group named ResourceGroup01, hosts the elastic pool in.
The command specifies the vCore property values for the pool and the databases in the pool.
...
The default values for the different editions are as follows: - Basic.
5 DTUs
...
100 DTUs - Standard.
100 DTUs - Premium.
125 DTUs For details about which values are valid, see the table for your specific size pool in elastic pools (https://learn.microsoft.com/azure/sql-database/sql-database-elastic-pool).
...
### -Tags
Specifies a dictionary of Key-value pairs in the form of a hash table that this cmdlet associates with the elastic pool.
