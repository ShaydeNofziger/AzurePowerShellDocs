...
## DESCRIPTION
Use the Update-AzServiceFabricNodeType to update node type related settings.
When a node type to stop acting as a primary node type using '-IsPrimaryNodeType $false', another primary node type must already be registered within the cluster.
...
This command will update NodeType 'nt1' to stop acting as a primary node type.
It may be done after adding another primary node type, in order to migrate seed nodes and system services.
