...
This command creates a Storage account with Kind StorageV2. 
It also generates and assigns an identity that can be used to manage account keys through Azure KeyVault.
...
Possible values include: 'User', 'Computer'.
...
This parameter must be set when -EnableActiveDirectoryDomainServicesForFile is set to true.
...
### -AllowedCopyScope
Set restrict copy to and from Storage Accounts within an AAD tenant or with Private Links to the same VNet.
Possible values include: 'PrivateLink', 'AAD'
...
Set the Encryption KeyType for Queue.
...
Set the Encryption KeyType for Table.
The default value is Service.
...
Possible values include: 'Unlocked', 'Disabled. 
...
The acceptable values for this parameter are: - Storage.
