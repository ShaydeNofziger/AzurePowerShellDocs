...
With the \`Expand\` flag, one can navigate through the \`Children\` array and get details for each child.
For example, \`Children\[0\]\` will give details for the group with display name \`TestGroup1DisplayName\`.
