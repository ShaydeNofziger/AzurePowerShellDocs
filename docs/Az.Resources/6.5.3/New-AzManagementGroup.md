...
## DESCRIPTION
The New-AzManagementGroup cmdlet creates a management group with name GroupName .
The DisplayName and ParentId can also be set when creating a new Management Group.
...
Creation of a new group with \`DisplayName\` and \`ParentId\` set to \`null\`.
The \`DisplayName\` will be same as the \`GroupName\` and the parent of the group will be the tenant.
