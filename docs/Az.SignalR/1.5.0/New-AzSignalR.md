...
## DESCRIPTION
Create a SignalR service.
The following values will be used for the parameters if not specified: * \`ResourceGroupName\`: the default resource group set by \`Set-AzDefault -ResourceGroupName\`.
...
### -AllowedOrigin
The allowed origins for the SignalR service.
To allow all, use "*" and remove all other origins from the list.
Slashes are not allowed as part of domain or after TLD
